...
def generate_image(frame, netG):
    noise = torch.randn(BATCH_SIZE, 784)
    if use_cuda:
    ...
    lib.save_images.save_images(
        samples,
        'tmp/mnist_test/samples_{}.png'.format(frame)
    )
...
for iteration in xrange(ITERS):
    ...
    for iter_d in xrange(CRITIC_ITERS):
        ...
        # train with fake
        noise = torch.randn(BATCH_SIZE, 784)
        #noise = torch.Tensor(_data[:,:392])
        tag   = torch.Tensor(_data[:,392:])
        if use_cuda:
            noise = noise.cuda(gpu)
            tag   = tag.cuda(gpu)
        noisev = autograd.Variable(noise, volatile=True)  # totally freeze netG
        #fake = autograd.Variable(torch.cat((netG(noisev).data,tag),1))
    ...
    noise = torch.randn(BATCH_SIZE, 392)
    if use_cuda:
        noise = noise.cuda(gpu)
